...
public final class FactionClaimBorderRenderer {
    ...
    public static void render(RenderLevelStageEvent event) {
        ...
        for (Map.Entry<Long, FactionClaimMapPacket.ClaimEntry> entry : snapshot.claims().entrySet()) {
            ...
            drawVerticalQuad(consumer, minX, minZ, minX, maxZ, minY, maxY, red, green, blue, BORDER_ALPHA,
                u0, u1, v0, v1);
            drawVerticalQuad(consumer, maxX, minZ, maxX, maxZ, minY, maxY, red, green, blue, BORDER_ALPHA,
                u0, u1, v0, v1);
            drawVerticalQuad(consumer, minX, minZ, maxX, minZ, minY, maxY, red, green, blue, BORDER_ALPHA,
                u0, u1, v0, v1);
            drawVerticalQuad(consumer, minX, maxZ, maxX, maxZ, minY, maxY, red, green, blue, BORDER_ALPHA,
                u0, u1, v0, v1);
        }
    ...
    private static void drawVerticalQuad(VertexConsumer consumer, double x1, double z1, double x2, double z2,
                                         double minY, double maxY, float red, float green, float blue, float alpha,
                                         float u0, float u1, float v0, float v1) {
        consumer.vertex((float) x1, (float) minY, (float) z1).color(red, green, blue, alpha).uv(u0, v1).endVertex();
        consumer.vertex((float) x1, (float) maxY, (float) z1).color(red, green, blue, alpha).uv(u0, v0).endVertex();
        consumer.vertex((float) x2, (float) maxY, (float) z2).color(red, green, blue, alpha).uv(u1, v0).endVertex();
        consumer.vertex((float) x2, (float) minY, (float) z2).color(red, green, blue, alpha).uv(u1, v1).endVertex();
    }
}
